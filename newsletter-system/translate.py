...
FLAG = {
    "미국": "🇺🇸", "캐나다": "🇨🇦", "영국": "🇬🇧", "프랑스": "🇫🇷", "독일": "🇩🇪",
    "폴란드": "🇵🇱", "노르웨이": "🇳🇴", "핀란드": "🇫🇮", "스웨덴": "🇸🇪", "덴마크": "🇩🇰",
    "이탈리아": "🇮🇹", "스페인": "🇪🇸", "포르투갈": "🇵🇹", "네덜란드": "🇳🇱",
    "벨기에": "🇧🇪", "오스트리아": "🇦🇹", "스위스": "🇨🇭", "그리스": "🇬🇷",
    "루마니아": "🇷🇴", "불가리아": "🇧🇬", "체코": "🇨🇿", "슬로바키아": "🇸🇰",
    "슬로베니아": "🇸🇮", "헝가리": "🇭🇺", "크로아티아": "🇭🇷", "에스토니아": "🇪🇪",
    "라트비아": "🇱🇻", "리투아니아": "🇱🇹", "아일랜드": "🇮🇪", "룩셈부르크": "🇱🇺",
    "몰타": "🇲🇹", "키프로스": "🇨🇾", "아이슬란드": "🇮🇸", "우크라이나": "🇺🇦",
    "호주": "🇦🇺", "인도": "🇮🇳", "인도네시아": "🇮🇩", "말레이시아": "🇲🇾",
    "필리핀": "🇵🇭", "베트남": "🇻🇳", "태국": "🇹🇭", "싱가포르": "🇸🇬",
    "일본": "🇯🇵", "대만": "🇹🇼", "한국": "🇰🇷", "대한민국": "🇰🇷",
...
# 국가명 외 자주 나오는 표현 → 국가
_FLAG_EXTRA = [
    (r"\b(u\.s\.|us army|us navy|us air force|pentagon|american)\b", "미국"),
    (r"미 육군|미 해군|미 공군|미군|펜타곤", "미국"),
    (r"\b(royal navy|royal air force|british|mod uk)\b", "영국"),
    (r"\b(bundeswehr)\b", "독일"),
    (r"\b(nato)\b|나토", "EU"),
]
...
def flag_for(text):
    """제목에서 기사가 다루는 국가를 찾아 국기 이모지를 돌려준다. 없으면 ''."""
    t = (text or "")
    tl = t.lower()
    for en, ko in COUNTRY.items():
        if not en.isascii():
            continue
        if re.search(r"(?<![a-z])" + re.escape(en) + r"(?![a-z])", tl):
            return FLAG.get(ko, "")
    for ko_name, fl in FLAG.items():
        if ko_name in t:
            return fl
    for pat, ko in _FLAG_EXTRA:
        if re.search(pat, tl):
            return FLAG.get(ko, "")
    return ""
