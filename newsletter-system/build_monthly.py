...
RAW = os.path.join(ROOT, "data", "archive", "raw_monthly")
ED = os.path.join(ROOT, "data", "archive", "edition_monthly")
...
def build_one(ed):
    ...
    for it in sorted(items, key=lambda x: x.get("deadline") or "9999"):
        ...
        try:
            ...
            continue
        if pub_d <= dd <= pub_d + timedelta(days=45):
            ...
            watch.append({"country": ko_country(c),
                          ...
                          "urgent": dd <= pub_d + timedelta(days=14), "url": it["url"]})
        if len(watch) >= 8:
            break

    if feats:
        brief_secs.append({"kind": "feature", "title": "지난달 핵심",
                           "subtitle": "한 달의 흐름을 사업기회 관점에서 정리 — 다음 달 대응 방향까지 — "
                                       "왜 중요한가 · 누구에게 기회인가 · 지금 할 일",
                           "items": feats})

    if watch:
        brief_secs.append({"kind": "table", "title": "이번 달 주목 일정",
                           "subtitle": "발행일 기준 45일 내 마감되는 공고 — 이번 달 안에 검토해야 대응이 가능합니다",
                           "items": watch})
    ...
    tabs = [
        ...
        {"id": "mice", "label": "전시회·MICE 캘린더",
         ...
         "sections": [
             ...
             {"kind": "scrap", "title": "지난달 전시회·상담회 소식",
    ...
    doc = {
        ...
        "cadence": "월간뉴스", "lead_title": "이번 달 핵심",
    ...
    R.export_doc(doc, tabs, date, today,
                 os.path.join(OUT, f"{date}-m.json"))
    R.og_shell(doc, ed["date"] + "-m", os.path.join(os.path.dirname(OUT), "archive"))
...
def main():
    ...
    for f in files:
        ...
        try:
            ...
            continue
        open(os.path.join(OUT, f"{ed['date']}-m.html"), "w", encoding="utf-8").write(mail)
        open(os.path.join(OUT, f"{ed['date']}-m_web.html"), "w", encoding="utf-8").write(web)
        index.append({
            "slug": ed["date"] + "-m", "type": "monthly",
